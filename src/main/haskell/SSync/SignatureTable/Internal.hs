 {-# LANGUAGE CPP, RankNTypes, ScopedTypeVariables, DeriveDataTypeable, BangPatterns, RecordWildCards, NamedFieldPuns, MultiWayIf, LambdaCase, OverloadedStrings, ViewPatterns #-}

module SSync.SignatureTable.Internal (
  SignatureTable
, smallify
, stChecksumAlg
, stBlockSize
...
getSignatureTable :: HashAlgorithm -> ExceptT SignatureTableException Get SignatureTable
getSignatureTable checksumAlg = do
  bs <- getVarInt
  when (bs > maxBlockSize) $ throwError (InvalidBlockSize bs)
  ...
  return $ ST bs (fromIntegral bs) checksumAlg strongHashAlg blocksSorted blocksIndexed
...
consumeSignatureTable = orThrow $ do
  ...
  (st, d) <- withHashTEx checksumAlg $ do
    st <- consumeAndHash $ getSignatureTable checksumAlg
    d <- lift digestS
    return (st, d)
